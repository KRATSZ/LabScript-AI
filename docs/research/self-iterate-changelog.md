...
Historical notes from on-robot validation runs (IPs, software versions, and scenario checklists). **Current policy and error handling are defined in `docs/rules/error-response.md` and `docs/rules/safety-policy.md`, not in this section.**
...
6. `docs/research/self-iterate-changelog.md` — This changelog
...
- **Fix:** Updated `CLAUDE.md`, `docs/rules/workflows.md`, `skills/opentrons-protocol-author/SKILL.md`, and `references/authoring-appendix.md` so the agent can stop after validation or labware inspection when the user only asked for that.
