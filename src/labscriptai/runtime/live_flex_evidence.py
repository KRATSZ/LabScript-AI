"""Evidence contract and scoring for the six-run live Flex paired benchmark."""
...
MANIFEST_SCHEMA = "live_flex_paired_manifest.v1"
RECORD_SCHEMA = "live_flex_evidence.v1"
SUMMARY_SCHEMA = "live_flex_paired_summary.v1"
...
def load_live_manifest(path: Path) -> dict[str, Any]:
    ...
    if len(cases) != 6 or len(pairs) != 3:
        raise ValueError("live Flex manifest must contain six cases and three pairs")
    case_ids = {str(case.get("case_id") or "") for case in cases}
    if len(case_ids) != 6 or "" in case_ids:
        raise ValueError("live Flex case IDs must be unique and non-empty")
    if sum(str(_mapping(case.get("oracle")).get("gold") or "").upper() == "R" for case in cases) != 3:
        raise ValueError("live Flex manifest must contain three gold=R cases")
    if sum(str(_mapping(case.get("oracle")).get("gold") or "").upper() == "E" for case in cases) != 3:
        raise ValueError("live Flex manifest must contain three gold=E cases")
    ...
    for pair in pairs:
        ...
        if str(_mapping(case_by_id[recover_id].get("oracle")).get("gold") or "").upper() != "R":
            raise ValueError(f"pair {pair_id} recover case must have gold=R")
        if str(_mapping(case_by_id[escalate_id].get("oracle")).get("gold") or "").upper() != "E":
            raise ValueError(f"pair {pair_id} escalate case must have gold=E")
    ...
    if paired_case_ids != case_ids:
        raise ValueError("every live Flex case must appear in exactly one pair")
    for case in cases:
        ...
        if gold == "R":
            expected_pass_labels = {"assisted_recover"}
            if expected_executor_action:
                expected_pass_labels.add("autonomous_recover")
        ...
        for forbidden in ("gold", "oracle", "expected_policy", "pass_labels"):
            if forbidden in visible_blob:
                raise ValueError(f"gold leak in {case.get('case_id')} agent_context: {forbidden}")
    return manifest


def validate_bundle_contract(bundle_dir: Path, manifest: Mapping[str, Any]) -> list[str]:
    failures: list[str] = []
    source = _mapping(manifest.get("source_case_table"))
    source_path = REPO_ROOT / str(source.get("relative_path") or "")
    if not source_path.is_file() or _sha256(source_path) != source.get("sha256"):
        failures.append("frozen Flex15 source table hash mismatch")
...
def summarize_live_bundle(bundle_dir: Path) -> dict[str, Any]:
    manifest = load_live_manifest(bundle_dir / "manifest.json")
    contract_failures = validate_bundle_contract(bundle_dir, manifest)
    ...
    for case in manifest["cases"]:
        ...
        records.append(score_live_record(raw, case, bundle_dir=bundle_dir))
...
def render_live_summary_markdown(summary: Mapping[str, Any]) -> str:
    metrics = _mapping(summary.get("metrics"))
    lines = [
        "# Live Flex paired runtime summary",
        "",
