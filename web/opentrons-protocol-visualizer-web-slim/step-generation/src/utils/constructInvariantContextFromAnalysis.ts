...
export function constructInvariantContextFromAnalysis(
  ...
  const labware = analysis.labware ?? []
  const modules = analysis.modules ?? []
  const pipettes = analysis.pipettes ?? []
  const commands = analysis.commands ?? []
  const liquids = analysis.liquids ?? []
  ...
  const pipetteEntities = pipettes.reduce<PipetteEntities>((acc, pipette) => {
    ...
    const matchingLabwareEntities = tiprackIdsAssosciatedWithPipette
      .map(
        pickUpTipCommand =>
          labwareEntitiesWithLidStacks[pickUpTipCommand.params.labwareId]
      )
      .filter((entity): entity is LabwareEntity => entity != null)
