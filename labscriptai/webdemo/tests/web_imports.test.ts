...
describe("AnimationOverlay code-split", () => {
  ...
  it("vite uses npm Opentrons viz and optional GitHub hang, not a vendored slim", () => {
    ...
    assert.doesNotMatch(css, /@keyframes bubble-in/);
    ...
    assert.match(css, /robot_coords_foreign_object_container/);
    assert.match(css, /transform:\s*scaleY\(-1\)/);
    assert.doesNotMatch(css, /robotcoordinatespace-module__svg/);
    const app = readFileSync(path.join(webSrc, "App.tsx"), "utf8");
    ...
    const otDeck = readFileSync(path.join(webSrc, "OtDeckReplay.tsx"), "utf8");
    assert.match(otDeck, /is-flex/);
    assert.match(otDeck, /FlexReplayTicks/);
