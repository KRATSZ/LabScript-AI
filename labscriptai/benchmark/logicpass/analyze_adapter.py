...
def _dispose_command(
    ...
    *,
    ...
    if command_type in STATE_HOLDING_ZERO_TRANSFER_COMMANDS:
        # Blowout + mix: record the command, model zero net transferred volume.
        # Mix is liquid-contact (not touchTip); a mix parent is not consumed by
        # the ledger, so tip dirtiness is not updated from the parent itself.
        # PE 8.8.1 expands pipette.mix to aspirate/dispense leaves instead.
        if status is None:
            return _lp_l5("missing_status")
        if status != "succeeded":
            return _lp_l5("status_not_succeeded")
