...
def run(ctx):
    ...
    # 固定布局：Left Mount: 1-Channel, Right Mount: 8-Channel
    p1k_1_loc = "left"
    p1k_8_loc = "right"

    # deck layout - 固定布局
    # Extension Mount: Flex Gripper (自动可用，无需加载)
    ...
    # C1: Temperature Module GEN2
    temp = ctx.load_module("Temperature Module Gen2", "C1")
    temp_adapter = temp.load_adapter("opentrons_aluminum_flat_bottom_plate")
    
    # A1+B1: Thermocycler Module GEN2
    tc = ctx.load_module("thermocyclerModuleV2")
    
    # D2: Magnetic Block GEN1
    ...
    # 根据条件分配tip rack和sample rack
    if dilution_on_deck == 1:
        # 稀释模式：需要200ul tips用于稀释
        tips_200 = ctx.load_labware("opentrons_flex_96_tiprack_200ul", "C3", "P200 TIPS")
        tips_200_loc = tips_200.wells()[:96]
        
        # sample rack放在D3，为working_plate在C2预留空间
        sample_rack_1 = ctx.load_labware(
            "opentrons_24_tuberack_nest_1.5ml_snapcap", "D3", "SAMPLES"
        )
        # 暂时注释以简化布局
        # if num_sample_final > 24:
        #     sample_rack_2 = ctx.load_labware(
        #         "opentrons_24_tuberack_nest_1.5ml_snapcap", "A3", "SAMPLES"
        #     )
    else:
        # 非稀释模式
        tips_200 = ctx.load_labware("opentrons_flex_96_tiprack_200ul", "C3", "P200 TIPS")
    ...
    if dilution_on_deck == 1:
        ...
        if num_sample_initial > 6:
            def_undiluted_1 = ctx.define_liquid(
                name="Sample",
                description="Undiluted Samples, volume per tube (Slot C2)",
                display_color="#FF0000",
            )  ## Red
            def_undiluted_2 = ctx.define_liquid(
                name="Sample",
                description="Undiluted Samples, volume per tube (Slot D3)",
                display_color="#FF0000",
            )  ## Red
            for p in range(6):
        ...
        if num_sample_final > 24:
            def_1 = ctx.define_liquid(
                name="Sample",
                description="Samples, volume per tube (Slot C2)",
                display_color="#FF0000",
            )  ## Red
            def_2 = ctx.define_liquid(
                name="Sample",
                description="Samples, volume per tube (Slot D3)",
                display_color="#FF0000",
            )  ## Red
            for p in range(24):
    ...
    del ctx.deck["C2"]
    # 移除sample_rack_1释放D3位置
    del ctx.deck["D3"]
    # 在此时加载working_plate_lid到空闲位置
    working_plate_lid = ctx.load_labware(
        "corning_96_wellplate_360ul_flat", "D3", "PLATE LID"
    )
    ctx.move_labware(
