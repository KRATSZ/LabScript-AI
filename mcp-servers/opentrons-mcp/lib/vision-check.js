...
const visionScriptPath = path.resolve(__dirname, "../scripts/vision_check.py");
...
async function runVisionCheckPython(payload, preferredPython) {
  const candidates = [];
  const detectedLocalPython = fs.existsSync(repoLocalPythonPath) ? repoLocalPythonPath : null;
