"""Flex protocol — Strict-window enzyme add.
...
metadata = {
    "protocolName": "Strict-window enzyme add",
    "author": "LabscriptAI OT",
    "description": "Enzyme-like additive must be added shortly after buffer contacts the sample.",
}
...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    protocol.comment("Phase 2: Enzyme additive -> D2.A1")
    pipette.pick_up_tip()
    if use_liquid_probe:
        pipette.require_liquid_presence(enzyme)
    pipette.transfer_with_liquid_class(
        liquid_class=glycerol, volume=50, source=enzyme, dest=sample,
        new_tip="never", trash_location=trash,
    )
    finish_tip(pipette, trash, dry_run_on)
