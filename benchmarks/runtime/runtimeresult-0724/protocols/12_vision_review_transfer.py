"""Flex protocol — Vision review transfer.
...
metadata = {
    "protocolName": "Vision review transfer",
    "author": "LabscriptAI OT",
    "description": "Require a clear deck review before a simple buffer transfer.",
}
...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    protocol.comment("Deck appearance review required before motion")
    try:
        protocol.capture_image(filename="decision_bench_12_prereview")
    except Exception:
        protocol.comment("capture_image unavailable; operator/agent must still review deck clarity")
