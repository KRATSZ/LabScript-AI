"""Flex protocol — Hold-tolerant two-phase prep.
...
metadata = {
    "protocolName": "Hold-tolerant two-phase prep",
    "author": "LabscriptAI OT",
    "description": "Two-phase prep where a brief hold between phases is acceptable.",
}
...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    protocol.comment("Phase 1: Assay Buffer -> D2.A1")
    pipette.pick_up_tip()
    if use_liquid_probe:
    ...
    protocol.comment(
        "Hold point: brief pause of a few minutes between phases is acceptable"
    )

    protocol.comment("Phase 2: Additive -> D2.A1")
    pipette.pick_up_tip()
    if use_liquid_probe:
