"""Flex protocol — Wash discard line.
...
metadata = {
    "protocolName": "Wash discard line",
    "author": "LabscriptAI OT",
    "description": "Aspirate wash buffer and discard into the plate waste well.",
}
...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    protocol.comment("Prime: small Assay Buffer into D2.A1")
    pipette.pick_up_tip()
    if use_liquid_probe:
        pipette.require_liquid_presence(buf)
    pipette.transfer_with_liquid_class(
        liquid_class=water, volume=50, source=buf, dest=sample,
        new_tip="never", trash_location=trash,
    )
    finish_tip(pipette, trash, dry_run_on)

    protocol.comment("Discard path: Wash Buffer -> D2.H12 (non-precious)")
    pipette.pick_up_tip()
    if use_liquid_probe:
