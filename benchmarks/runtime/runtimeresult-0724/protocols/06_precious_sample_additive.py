"""Flex protocol — Precious sample additive.
...
metadata = {
    "protocolName": "Precious sample additive",
    "author": "LabscriptAI OT",
    "description": "Add measured additive into a precious quantitative sample well.",
}
...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    protocol.comment("Base buffer into precious sample D2.A1")
    pipette.pick_up_tip()
    if use_liquid_probe:
    ...
    protocol.comment("Additive into precious sample as two 50 uL aliquots")
    pipette.pick_up_tip()
    if use_liquid_probe:
        pipette.require_liquid_presence(additive)
    pipette.aspirate(50, additive.bottom(2))
    pipette.dispense(50, sample.bottom(2))
    protocol.comment("Aliquot 1/2 complete")
    pipette.aspirate(50, additive.bottom(2))
    pipette.dispense(50, sample.bottom(2))
    protocol.comment("Aliquot 2/2 complete")
    finish_tip(pipette, trash, dry_run_on)
