"""Flex protocol — Sample confirm then stock add.
...
metadata = {
    "protocolName": "Sample confirm then stock add",
    "author": "LabscriptAI OT",
    "description": "Confirm liquid in sample well, then add more Assay Buffer from common stock.",
}
...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    protocol.comment("Seed sample with Assay Buffer")
    pipette.pick_up_tip()
    if use_liquid_probe:
        pipette.require_liquid_presence(buf)
    # ≥150 µL so LPD can reliably detect liquid in nest_96 flat wells
    # (60 µL seed previously died at Confirm and never reached the tip-policy decision).
    pipette.transfer_with_liquid_class(
        liquid_class=water, volume=150, source=buf, dest=sample,
        new_tip="never", trash_location=trash,
    )
    finish_tip(pipette, trash, dry_run_on)

    protocol.comment("Confirm liquid in sample D2.A1")
    pipette.pick_up_tip()
    if use_liquid_probe:
        pipette.require_liquid_presence(sample)
        protocol.comment("LPD: sample well contacted")
    # Happy path uses a fresh tip for stock re-entry below.
    finish_tip(pipette, trash, dry_run_on)

    protocol.comment("Add more Assay Buffer from common stock C2.A1")
    pipette.pick_up_tip()
    if use_liquid_probe:
        pipette.require_liquid_presence(buf)
    pipette.transfer_with_liquid_class(
        liquid_class=water, volume=60, source=buf, dest=sample,
        new_tip="never", trash_location=trash,
    )
    finish_tip(pipette, trash, dry_run_on)
