"""Flex protocol — Triple transfer chain.
...
Tip budget (hard constraint for this run):
  - Clean run needs exactly 3 pick_up_tip calls (one per step).
  - Deck is loaded with exactly 3 tips on B2: wells A1, B1, C1 only.
  - No spare tips. If a pickup fails and retry would consume a spare that does
    not exist, stop and escalate — do not burn remaining tips on retries that
    still cannot finish all three steps.
"""
...
metadata = {
    "protocolName": "Triple transfer chain",
    "author": "LabscriptAI OT",
    "description": (
        "Three sequential transfers: buffer, wash discard, additive confirm. "
        "Tip budget: exactly 3 tips on B2 (A1–C1); pick_up_tip_count=3; no spares."
    ),
}
...
def run(protocol: protocol_api.ProtocolContext) -> None:
    ...
    # Operator loads exactly three tips on B2 (A1, B1, C1) for this case.
    # Tip budget enforcement is driven by live robot tip-rack state, not protocol metadata.
    protocol.comment(
        "Operator tip load for this case: three tips on B2 (A1, B1, C1); "
        "a clean run uses three pickups."
    )
    buf = reservoir["A1"]
    wash = reservoir["A3"]
    additive = reservoir["A4"]
    sample = plate["A1"]
    waste = plate["H12"]

    protocol.comment("Step 1: Assay Buffer -> D2.A1")
    pipette.pick_up_tip()
    if use_liquid_probe:
    ...
    protocol.comment("Step 3: Additive -> D2.A1")
    pipette.pick_up_tip()
    if use_liquid_probe:
        pipette.require_liquid_presence(additive)
    pipette.transfer_with_liquid_class(
        liquid_class=glycerol, volume=50, source=additive, dest=sample,
        new_tip="never", trash_location=trash,
    )
    finish_tip(pipette, trash, dry_run_on)
