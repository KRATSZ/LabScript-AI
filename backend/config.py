...
# LLM request timeout in seconds (REQUEST_TIMEOUT env, default 300).
# Reasoning models (e.g. deepseek-v4-flash) can spend minutes in the thinking
# phase before producing content, so per-call timeouts must be generous;
# aligned with the Nginx proxy_read_timeout (300s) budget.
REQUEST_TIMEOUT = _get_int_env("REQUEST_TIMEOUT", default=300)
...
REVIEW_PRIMARY_MODEL_NAME = model_name
REVIEW_VISION_TOOL_CONFIG = {
    "model": _get_env("LABSCRIPTAI_VISION_MODEL", default="zai-org/GLM-4.5V"),
    "base_url": _strip_trailing_slash(_get_env("LABSCRIPTAI_VISION_BASE_URL", default="https://api.siliconflow.cn/v1")),
    "api_key": _get_env("LABSCRIPTAI_VISION_API_KEY", default=""),
}
